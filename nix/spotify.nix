{
  ...
  # the wrapper that loads our mod
  # can be found in /deps/wrapper
  # built by ./wrapper.nix
  extendifyWrapper ? throw "extendifyWrapper is required",
  # alternate cef to use
  # eg: debugging info
  cef ? null,
}:
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    patchelf \
      --add-needed libextendify_wrapper.so \
      $out/share/spotify/spotify
      
    ln -s ${extendifyWrapper}/lib/libextendify_wrapper.so $libdir/libextendify_wrapper.so
  ...
  fixupPhase = ''
    ...
    wrapProgramShell $out/share/spotify/spotify \
      ...
      ${lib.optionalString (cef != null) ''
        cp ${cef}/lib/*.so $out/share/spotify/
      ''}
    runHook postFixup
  '';
